...
function createServerHmrApplier(
  ...
  let pendingApply: Promise<boolean> = Promise.resolve(false)

  // Resolves to whether the server's render output can differ from what
  // browsers currently hold, i.e. whether they need to refetch RSC.
  return function applyServerHmrUpdate(): Promise<boolean> {
    const apply = pendingApply.then(async () => {
      ...
      // until the next request. The next render loads from disk, so browsers
      // still need to refetch (e.g. to clear a redbox after a failed build).
      ...
      // An empty partial only advances the native version state. Nothing
      // changed, so browsers must not be told to refetch: a refetch itself
      // pulls again, which would ping-pong forever.
    ...
    pendingApply = apply.catch(() => false)
    return apply
  }
}
...
export async function createHotReloaderTurbopack(
  ...
  // Applies any pending server HMR update and reports whether browsers need to
  // refetch RSC. Without server fast refresh nothing is applied in-process, so
  // a change notification always means they do.
  const applyServerHmrUpdate: () => Promise<boolean> = serverFastRefresh
  ...
  async function sendServerComponentChanges() {
    // A change notification can also come from an RSC refetch recompiling the
    // route, which carries no update to apply. Announcing that would trigger
    // another refetch, and so on.
    if (!(await applyServerHmrUpdate())) return
  ...
  async function handleEntrypointsUpdate(
    ...
    // Diff against the previous raw route keys. `currentEntrypoints` is keyed by
    // `originalName` instead, which for parallel routes never matches a raw key
    // (`/` vs `/@slot/page`), so every pull would report the route as added.
        ...
        .traceAsyncFn(async () => {
          ...
          try {
            ...
            if (
              ...
              // This request consumed the pending update, so it also owns
              // telling browsers to refetch.
              if (await applyServerHmrUpdate()) {
                notifyServerComponentChanges()
              }
            }
          } finally {
  ...
  if (entrypointsSubscription) {
    ...
    // `base-server`'s `prepare()` reads `actualInstrumentationHookFile` exactly
    // once and is never retried, and that field is only set while handling
    // entrypoints. Route compilation stays lazy; this just populates globals.
    await ensureEntrypoints()
  }
